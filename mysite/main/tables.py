...
from .models import Administrador, Utilizador
...
class UtilizadoresTable(django_tables.Table):
    ...
    class Meta:
        model = Utilizador
        sequence = ('nome', 'email', 'telefone', 'tipo', 'valido', 'acoes')
    ...
    def render_tipo(self, value):
        return "Docente" if value == "Docente" else value
    ...
    def render_acoes(self, record):
        ...
        if self.request.user != record.user_ptr and not (record.firstProfile == 'Administrador' and not self.request.user.groups.filter(name='Administrador').exists()):
            if record.valido == "Rejeitado":
                primeiro_botao = f"""
                <a data-tooltip="Validar" href="{reverse('main:validar', args=[record.first_name, record.id])}">
                    <span class="icon">
                ...
                primeiro_botao = f"""
                <a data-tooltip="Rejeitar" onclick="alert.render('Tem a certeza que pretende rejeitar este utilizador?','{reverse('main:rejeitar', args=[record.first_name, record.id])}')">
        ...
        if self.request.user.groups.filter(name='Administrador').exists():
            segundo_botao = f"""
            <a href='{reverse('main:alterar-utilizador-admin', args=[record.id])}'
                data-tooltip="Editar">
                <span class="icon">
            ...
            """
        terceiro_botao = ""
        if record.firstProfile == 'Funcionário':
            alerta = "Tem a certeza que pretende eliminar este utilizador?<br><br><b>Atenção!</b><br><br>A <b>incrição</b> deste Funcionário será apagada permanentemente."
        elif record.firstProfile == 'Docente':
            alerta = "Tem a certeza que pretende eliminar este utilizador?<br><br><b>Atenção!</b><br><br>As <b>atividades</b> pelo qual este Docente está responsável serão apagadas permanentemente."
        ...
        if self.request.user != record.user_ptr and terceiro_botao == "":
            terceiro_botao = f"""
                <a onclick="alert.render('{alerta}','{reverse('main:apagar-utilizador', args=[record.id])}')"
                    data-tooltip="Apagar">
                    <span class="icon has-text-danger">
                        <i class="mdi mdi-trash-can mdi-24px"></i>
                    </span>
